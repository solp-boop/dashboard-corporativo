...
    </div>
    <div>
        <p style='color:#64748b; font-size:9px; margin:0 0 2px 0;'> P2P</p>
...
                        dias_cons_val = " En preparación" if es_azul else (f"{round(avg_tcons, 1)} d" if pd.notna(avg_tcons) else "—")
...
                            )
                            st.info(" Azul gestiona cargas aéreas — los tiempos de consolidación marítima no aplican a su metodología.")
                            st.markdown("<br>", unsafe_allow_html=True)
                            st.dataframe(
...
                st.dataframe(df_show, use_container_width=True, hide_index=True, column_config={'T. Consol (días)': st.column_config.NumberColumn(format="%d días "), 'SLA': st.column_config.TextColumn("SLA Límite"), 'Tipo': st.column_config.TextColumn("Tipo Carga")})
...
            def tabla_aereos():
                st.info(" Módulo de alertas para cargas aéreas en desarrollo. Próximamente disponible.")

            render_alerta("aereos", " ", "ALERTA 7 — AÉREOS",
...
            st.error(" Para usar este chat flotante, necesitamos actualizar Streamlit. (Requiere versión 1.33 o superior).")
...
                if len(df_found) > 50:
                    st.warning(f" Se encontraron {len(df_found)} resultados. Procesando los primeros 50.")
...
                        val_sku = str(row.iloc[5]).strip() if len(row) > 5 else "—"  # col F
                        val_inv = str(row.iloc[17]).strip() if len(row) > 17 else "—"  # col R
...
    import traceback
    st.code(traceback.format_exc())Saludos/ Best Regards SOL PARRACoordinadora de Comercio Exterior Foreign Trade CoordinatorCel.: +54 (911) 6867 9832
Wechat:Solparra_
  www.bidcom.com.ar 
