...
# --- SOLAPA 1: ORIGEN ---
    with tabs[0]:
        try:
            ...
            with b1_col:
                st.markdown(f"<div style='{get_btn_style('inst')} border-radius:5px;'>")
            ...
            with b2_col:
                st.markdown(f"<div style='{get_btn_style('crit')} border-radius:5px;'>")
            ...
            with b3_col:
                st.markdown(f"<div style='{get_btn_style('rest')} border-radius:5px;'>")
            ...
            with b4_col:
                st.markdown(f"<div style='{get_btn_style('rank')} border-radius:5px;'>")
            ...
            with b5_col:
                st.markdown(f"<div style='{get_btn_style('estr')} border-radius:5px;'>")
                ...
                if f in ["inst", "crit", "rest"]:
                    ...
                    st.markdown(f"""<div style="background: rgba(255,255,255,0.02); padding: 25px; border-radius: 10px; border-left: 5px solid {color}; margin-bottom: 20px;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                            <div><p style="font-size:11px; color:#8899A6; margin:0;">CANTIDAD SO</p><p style="font-size:26px; font-weight:300; margin:0;">{len(dff)}</p></div>
                            <div><p style="font-size:11px; color:#8899A6; margin:0;">VOLUMEN TOTAL</p><p style="font-size:26px; font-weight:300; margin:0;">{int(round(dff['M3 Total'].sum())):,} M3</p></div>
                        </div></div>""", unsafe_allow_html=True)
                    st.dataframe(dff[['SO', 'Proveedor', 'M3 Total']], use_container_width=True)

                elif f == "rank":
                    col_rank = df.columns[1]
                    df_rank = df[pd.to_numeric(df[col_rank], errors='coerce') <= 100].sort_values(by=col_rank).copy()
                    st.dataframe(df_rank[['SO', col_rank, 'Proveedor', 'M3 Total']], use_container_width=True)

                elif f == "estr":
                    col_cp = df.columns[93]
                    df['Tipo_Carga'] = df[col_cp].apply(lambda x: 'MONOPROVEEDOR' if str(x).upper() == 'SI' else 'CONSOLIDADO')
                    st.dataframe(df[['SO', 'Proveedor', 'Tipo_Carga', 'M3 Total']], use_container_width=True)

            st.markdown("<br><hr style='opacity:0.1'><br>", unsafe_allow_html=True)

            # DISTRIBUCIÓN POR PAÍS
            st.markdown("<p style='color:#00a8ff; font-weight:700; letter-spacing:2px; font-size:14px; margin-bottom:20px;'>DISTRIBUCIÓN GEOGRÁFICA</p>", unsafe_allow_html=True)
            ...
            for pais, row in res_p.iterrows():
                ...
                c1.markdown(f"<p style='font-weight:700;'>{pais.upper()}</p>", unsafe_allow_html=True)
                c2.markdown(f"<p style='color:#00a8ff;'>{int(row['M3 Total']):,} M3</p>", unsafe_allow_html=True)
                c3.markdown(f"<p>{int(row['SO'])} SO</p>", unsafe_allow_html=True)
            ...
            with ga:
                ...
                fig_e = px.bar(etd_p, x='Mes_ETD_Full', y='M3 Total', title="PROYECCIÓN ETD (M3)", template='plotly_dark', color_discrete_sequence=['#00ff88'])
                st.plotly_chart(fig_e, use_container_width=True)
            with gb:
                eta_p = df.groupby('Mes_ETA_Full').agg({'M3 Total': 'sum'}).reset_index()
                fig_a = px.bar(eta_p, x='Mes_ETA_Full', y='M3 Total', title="PROYECCIÓN ETA (M3)", template='plotly_dark', color_discrete_sequence=['#ff4b4b'])
                st.plotly_chart(fig_a, use_container_width=True)

            # EQUIVALENTE CONTENEDORES
            ...
            st.markdown("<br><p style='text-align:center; color:#00a8ff; font-weight:700;'>EQUIVALENTE EN CONTENEDORES (BARCO)</p>", unsafe_allow_html=True)
            ca, cb = st.columns(2)
            with ca:
                etd_c = df_maritimo.groupby('Mes_ETD_Full').agg({'Contenedores': 'sum'}).reset_index()
                st.plotly_chart(px.bar(etd_c, x='Mes_ETD_Full', y='Contenedores', template='plotly_dark', color_discrete_sequence=['#00ff88']), use_container_width=True)
            with cb:
                eta_c = df_maritimo.groupby('Mes_ETA_Full').agg({'Contenedores': 'sum'}).reset_index()
                st.plotly_chart(px.bar(eta_c, x='Mes_ETA_Full', y='Contenedores', template='plotly_dark', color_discrete_sequence=['#ff4b4b']), use_container_width=True)
