...
# --- DISEÑO BIDCOM PREMIUM (Impacto Amplio) ---
st.markdown("""
    <style>
    ...
    .bicom-header h1 { font-size: 60px; letter-spacing: 8px; color: #ffffff; font-weight: 900; margin:0; }
    ...
    /* Métricas Masivas */
    .metric-container { text-align: center; padding: 40px; }
    .label-large { font-size: 18px; color: #808495; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 15px; }
    .value-massive { font-size: 110px; font-weight: 900; color: #ffffff; line-height: 1; margin: 0; text-shadow: 0 0 30px rgba(0,168,255,0.2); }
    .unit-text { font-size: 30px; color: #00a8ff; vertical-align: middle; margin-left: 10px; }

    /* Botones Interactivos con efecto Solapa Activa */
    .stButton { display: flex; justify-content: center; }
    .stButton>button {
        border-radius: 20px !important; color: white !important;
        width: 100%; height: 200px; transition: all 0.4s ease-in-out;
        font-weight: 800 !important; font-size: 24px !important;
        background: rgba(255, 255, 255, 0.03) !important;
        border: 1px solid rgba(255, 255, 255, 0.1) !important;
    }
    
    /* Hover: Se pone azul BIDCOM al acercarse o cliquear */
    .stButton>button:hover, .stButton>button:active, .stButton>button:focus {
        background-color: #003366 !important; 
        border: 2px solid #00a8ff !important;
        box-shadow: 0 0 40px rgba(0, 168, 255, 0.3) !important;
        transform: scale(1.03);
    }
    
    /* Bordes de color sutiles en reposo */
    div[data-testid="column"]:nth-child(2) .stButton > button { border-left: 8px solid #00ff88 !important; }
    div[data-testid="column"]:nth-child(3) .stButton > button { border-left: 8px solid #ff4b4b !important; }

    /* Resalte de Fila Total */
...
try:
    ...
    # --- HEADER ---
    st.markdown("<div class='bicom-header'><h1>BIDCOM</h1><p>MANAGEMENT LOGÍSTICO INTERNACIONAL</p></div>", unsafe_allow_html=True)

    tabs = st.tabs(["ORIGEN", "STATUS CARGAS", "INDICADORES", "AGENTES", "ANALISTAS", "FLETES"])

    with tabs[0]:
        # --- FILA 1: MÉTRICAS IMPACTO AMPLIO ---
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"<div class='metric-container'><p class='label-large'>CANTIDAD DE SO</p><p class='value-massive'>{int(cant_so)}</p></div>", unsafe_allow_html=True)
        with c2:
            st.markdown(f"<div class='metric-container'><p class='label-large'>VOLUMEN TOTAL</p><p class='value-massive'>{int(m3_totales):,}<span class='unit-text'>M3</span></p></div>", unsafe_allow_html=True)

        st.markdown("<br><hr style='opacity:0.1'><br>", unsafe_allow_html=True)

        # --- FILA 2: BOTONES INTERACTIVOS CENTRADOS ---
        ...
        st.markdown("<p style='text-align:center; letter-spacing:3px; color:#808495; font-weight:bold;'>ESTRUCTURA DE CARGA POR PAÍS</p>", unsafe_allow_html=True)
        ...
        # --- GRÁFICO DE PUERTOS ---
        st.markdown("<br><br><p style='text-align:center; letter-spacing:3px; color:#ffffff; font-weight:bold; font-size:20px;'>DISTRIBUCIÓN POR PUERTO DE SALIDA</p>", unsafe_allow_html=True)
    ...
    st.error(f"Falla crítica en datos: {e}")
