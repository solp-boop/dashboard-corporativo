...
# ==========================================
    ...
    with tabs[1]:
        try:
            # 1. CARGA DE DATOS (Planif Cargas)
            ...
            # 2. FILTRADO: Solo lo que tiene Fecha de Instrucción (Columna U - Índice 20)
            # Limpiamos la columna de fecha para asegurar que sea válida
            df_res['Fecha_Inst_Valid'] = pd.to_datetime(df_res.iloc[:, 20], dayfirst=True, errors='coerce')
            df_inst = df_res[df_res['Fecha_Inst_Valid'].notna()].copy()

            # --- BLOQUE 1: KPIs GRANDES (CORREGIDOS) ---
            st.markdown("<br>", unsafe_allow_html=True)
            k1, k2, k3 = st.columns(3)
            
            with k1:
                # SO: Columna A (Índice 0)
                cant_so = df_inst.iloc[:, 0].nunique()
            ...
            with k2:
                ...
                m3_val = pd.to_numeric(df_inst.iloc[:, 50].astype(str).str.replace(',', '.'), errors='coerce').sum()
                st.markdown(f"<div class='metric-container'><p style='font-size: 22px; color: #00a8ff; letter-spacing: 4px; font-weight: 700; margin-bottom: 0;'>VOLUMEN (M3)</p><p style='font-size: 90px; font-weight: 900; color: #00a8ff; line-height: 1; margin: 0; text-shadow: 0 0 25px rgba(0,168,255,0.4);'>{int(round(m3_val)):,}</p></div>", unsafe_allow_html=True)
            
            with k3:
                # Proveedor: Columna AE (Índice 30)
                cant_prov = df_inst.iloc[:, 30].nunique()
            ...
            df_inst['ETD_Status_K'] = df_inst.iloc[:, 10].astype(str).str.upper().str.strip()
            confirmados_glob = len(df_inst[df_inst['ETD_Status_K'] == "OK"])
            pendientes_glob = len(df_inst) - confirmados_glob
            p_ok_glob = round((confirmados_glob / len(df_inst) * 100)) if len(df_inst) > 0 else 0
            ...
            df_inst['F_ETD_Valid'] = pd.to_datetime(df_inst.iloc[:, 11], dayfirst=True, errors='coerce')

            # Prom. Gestión (Confirmados): ETD - Instrucción
            df_conf = df_inst[df_inst['ETD_Status_K'] == "OK"].copy()
            df_conf['diff_g'] = (df_conf['F_ETD_Valid'] - df_conf['Fecha_Inst_Valid']).dt.days
            p_gestion = df_conf['diff_g'].mean() if not df_conf['diff_g'].dropna().empty else 0

            # Prom. Espera (Pendientes): Hoy - Instrucción
            df_pend = df_inst[df_inst['ETD_Status_K'] != "OK"].copy()
            df_pend['diff_e'] = (hoy - df_pend['Fecha_Inst_Valid']).dt.days
            p_espera = df_pend['diff_e'].mean() if not df_pend['diff_e'].dropna().empty else 0

            st_c1, st_c2 = st.columns(2)
            with st_c1:
                st.markdown(f"""<div style="background: rgba(0, 168, 255, 0.05); padding: 20px; border-radius: 10px; border: 1px solid rgba(0, 168, 255, 0.2); text-align: center;">
                    <p style="color: #8899A6; font-weight: 700; margin: 0; font-size: 13px; letter-spacing: 1px;">PROM. GESTIÓN (INST-ETD)</p>
                    <p style="color: #00a8ff; font-weight: 300; margin: 0; font-size: 42px;">{int(round(p_gestion))} <span style="font-size:18px;">días</span></p>
                </div>""", unsafe_allow_html=True)
            with st_c2:
                st.markdown(f"""<div style="background: rgba(255, 75, 75, 0.05); padding: 20px; border-radius: 10px; border: 1px solid rgba(255, 75, 75, 0.2); text-align: center;">
                    <p style="color: #8899A6; font-weight: 700; margin: 0; font-size: 13px; letter-spacing: 1px;">PROM. ESPERA (PEND)</p>
                    <p style="color: #ff4b4b; font-weight: 300; margin: 0; font-size: 42px;">{int(round(p_espera))} <span style="font-size:18px;">días</span></p>
                </div>""", unsafe_allow_html=True)

            # (Continúa el resto del código de transporte y botones...)
            # ...
